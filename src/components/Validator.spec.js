...
describe('Validator Tests', () => {
  ...
  it('Should test for minLength', () => {
    assert.equal(Validator.validators.minLength.check(baseComponent, 5, 'Test.js'), false);
    assert.equal(Validator.validators.minLength.check(baseComponent, 4, 'Test.js'), true);
    assert.equal(Validator.validators.minLength.check(baseComponent, 3, 'Test.js'), true);
    assert.equal(Validator.validators.minLength.check(baseComponent, 6, 'Test.js'), false);
    assert.equal(Validator.validators.minLength.check(baseComponent, 6, ''), false);
  });

  it('Should test for maxLength', () => {
    assert.equal(Validator.validators.maxLength.check(baseComponent, 5, 'Test.js'), true);
    assert.equal(Validator.validators.maxLength.check(baseComponent, 4, 'Test.js'), true);
    assert.equal(Validator.validators.maxLength.check(baseComponent, 3, 'Test.js'), false);
    assert.equal(Validator.validators.maxLength.check(baseComponent, 6, 'Test.js'), true);
    assert.equal(Validator.validators.maxLength.check(baseComponent, 6, ''), true);
  });

  it('Should test for email', () => {
    assert.equal(Validator.validators.email.check(baseComponent, '', 'Test.js'), false);
  ...
  it('Should test for required', () => {
    ...
    assert.equal(Validator.validators.required.check(baseComponent, true, []), false);
    assert.equal(Validator.validators.required.check(baseComponent, true, ['Test.js']), true);
  });

  it('Should test for custom', () => {
    assert.equal(Validator.validators.custom.check(baseComponent, 'valid = (input == "test")', 'Test.js'), true);
    ...
    assert.equal(Validator.validators.custom.check(baseComponent, 'valid = (input == "test") ? true : "Should be false."', 'Test.js'), true);
  });
  ...
  it('Should test for pattern', () => {
    ...
    assert.equal(Validator.validators.pattern.check(baseComponent, 'w+', 'Test.js'), false);
    assert.equal(Validator.validators.pattern.check(baseComponent, '\\w+', 'Test.js'), true);
  ...
  it('Should test for json', () => {
    assert.equal(Validator.validators.json.check(baseComponent, {
      or: [{ '_isEqual': [{ var: 'data.Test.js' }, ['1', '2', '3']] }, 'Should be false.']
